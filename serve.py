...
# check if we're in a frozen environment (i.e. in a DMG)
...
def get_args():    
    ...
    parser.add_argument("-p", "--port", help="specify port to serve Drift from", type=int, required=True)
    parser.add_argument("-c", "--calc_intense", help="enable prosodic measures which use intensive Voxit calculations", action='store_true')
    parser.add_argument("-w", "--web", help="disable changing of settings through browser; use if hosting Drift as a website", action='store_true')
...
def main():
    ...
    if not is_free_port(DRIFT_PORT):
        print(f"Port {DRIFT_PORT} is in use, please choose another port for Drift4.", file=sys.stderr)
        return 1
    ...
    def _settings(cmd):
        nonlocal GENTLE_PORT, CALC_INTENSE, WEBSERVE
    ...
    def _measure(id=None, start_time=None, end_time=None, force_gen=None, raw=None):
        ...
        force_gen = bool(force_gen) if type(force_gen) is not str else force_gen.lower() == 'true'
        raw = bool(raw) if type(raw) is not str else raw.lower() == 'true'
