...
port = driftargs.port
root = secureroot.FolderlessRoot(port=port, interface="0.0.0.0", dirpath="public") if not (os.getenv("PRIVATE_KEY_FILENAME") and os.getenv("CERT_FILENAME")) \
    else secureroot.SecureRoot(port=port, interface="0.0.0.0", dirpath="public", key_path=os.getenv("PRIVATE_KEY_FILENAME"), crt_path=os.getenv("CERT_FILENAME"))
...
root.putChild(b"_stage", guts.Codestage(wwwdir="public"))
