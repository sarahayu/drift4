...
const useProsodicMeasures = ({ id, selection, docReady }) => {
    const {
        isSuccess: fullTSProsMeasuresSuccess, isFetching: fullTSProsMeasuresFetching, data: fullTSProsMeasures
    } = useQuery(['prosodicMeasures', 'fullTranscript', id], () => getMeasureFullTS(id), { enabled: docReady });

    const {
        isSuccess: selectionProsMeasuresSuccess, isFetching: selectionProsMeasuresFetching, data: selectionProsMeasures
